...
print('-----idx2word=',idx2word[torch.tensor(2).item()])
print("------idx2word=",idx2word[3])
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        '''
        print("-----------in decoder,dec_inputs=",dec_inputs)
...
model=torch.load("./transformer_d128.pth")
################ Test Phase ####################################
def greedy_decoder(model, enc_input, start_symbol):
    """
    For simplicity, a Greedy Decoder is Beam search when K=1. This is necessary for inference 
    as we don't know the  target sequence input. Therefore we try to generate the target input
    word by word, then feed it into the transformer.
    ...
    return dec_input
# Test
enc_inputs=torch.tensor([[1, 2, 3, 5, 0]])
# enc_inputs=torch.tensor([[1, 2, 3, 4, 0]])
# enc_inputs=torch.tensor([[1, 6, 7, 8, 0]]) 
